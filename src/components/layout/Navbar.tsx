...
const Navbar = () => {
  ...
  const handleTabClick = (tabName: string) => {
    if (tabName === 'Search') {
      ...
      if (routes[tabName]) router.push(routes[tabName]);
    }
  };
  ...
  const getNavbarPosition = () => {
    if (showSearchInput && keyboardOpen) return 'top-30';
    return 'bottom-6';
  };
  ...
  return (
    ...
    >
      <div className={`bg-gradient-to-tr from-black to-gray-700 rounded-full transition-all duration-500 ease-in-out ${
        showSearchInput ? 'px-4 py-3' : 'px-4 py-4'
      }`}>
        {showSearchInput ? (
            ...
            {navItems.map((item) => {
              ...
              return (
                <div
                  ...
                  className="flex-1 text-center cursor-pointer transition-all duration-300 hover:scale-101"
                >
                  {isActive ? (
                    <div className="flex items-center justify-center gap-1.5 sm:gap-2 bg-white rounded-full px-2 py-2 sm:px-3 transform transition-all duration-300 ease-out">
                      <Icon size={18} className="text-gray-900 transition-all duration-300" />
                      <span className="font-medium text-xs sm:text-sm text-gray-900 animate-fade-in">
                        {item.name}
                      </span>
                    </div>
                  ) : (
                    <div className="flex items-center justify-center p-2 transition-all duration-300">
                      <Icon size={20} className="text-gray-400 hover:text-gray-300 transition-colors duration-300" />
                    </div>
                  )}
                </div>
              );
